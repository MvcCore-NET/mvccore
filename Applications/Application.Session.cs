...
namespace MvcCore
{
    public partial class Application: IDisposable {
		...
		public void InitSession(HttpApplication app) {
			// Add event handlers.
			
			//app.AcquireRequestState += new EventHandler(this.OnAcquireRequestState);
			app.AuthorizeRequest += new EventHandler(this.OnAcquireRequestState);

			//app.ReleaseRequestState += new EventHandler(this.OnReleaseRequestState);
			app.PreSendRequestHeaders += new EventHandler(this.OnReleaseRequestState);

			// Create a SessionIDManager.
			this.pSessionIDManager = new System.Web.SessionState.SessionIDManager();
			this.pSessionIDManager.Initialize();
		...
		void ExpireCallback(object state) {
			try {
				this.pHashtableLock.AcquireWriterLock(Int32.MaxValue);

				this.RemoveExpiredSessionData();

			} finally {
				this.pHashtableLock.ReleaseWriterLock();
			}
		}
		...
		private void OnAcquireRequestState(object source, EventArgs args) {
			...
			this.pSessionIDManager.InitializeRequest(context, false, out supportSessionIDReissue);
			sessionID = this.pSessionIDManager.GetSessionID(context);


			if (sessionID != null) {
				try {
					this.pHashtableLock.AcquireReaderLock(Int32.MaxValue);
					sessionData = (SessionItem)this.pSessionItems[sessionID];

					if (sessionData != null)
						sessionData.Expires = DateTime.Now.AddMinutes(pTimeout);
				} finally {
					this.pHashtableLock.ReleaseReaderLock();
				}
			} else {
				bool redirected, cookieAdded;

				sessionID = this.pSessionIDManager.CreateSessionID(context);
				this.pSessionIDManager.SaveSessionID(context, sessionID, out redirected, out cookieAdded);
			...
			if (sessionData == null) {
				...
				try {
					this.pHashtableLock.AcquireWriterLock(Int32.MaxValue);
					this.pSessionItems[sessionID] = sessionData;
				} finally {
					this.pHashtableLock.ReleaseWriterLock();
				}
			}
			...
			SessionStateUtility.AddHttpSessionStateToContext(
				context, new HttpSessionStateContainer(
					sessionID, sessionData.Items, sessionData.StaticObjects,
					pTimeout, isNew, pCookieMode, SessionStateMode.Custom, false
				)
			);

			// Execute the Session_OnStart event for a new session.
			if (isNew && this.Start != null) {
